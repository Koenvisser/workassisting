...
impl<const ATOMICS: usize, const MIN_CHUNKS: usize, const CHUNK_SIZE: usize> SchedulerTrait for Scheduler<ATOMICS, MIN_CHUNKS, CHUNK_SIZE> {
  ...
  fn get_name() -> String {
    format!("Multi-atomics 3 {} {} {}", ATOMICS, MIN_CHUNKS, CHUNK_SIZE)
  }

  fn get_chart_line_style() -> ChartLineStyle {
    ChartLineStyle::Dynamic(CHUNK_SIZE, MIN_CHUNKS, ATOMICS)
  }
}
...
impl<'a, const ATOMICS: usize, const MIN_CHUNKS: usize> Workers<'a, ATOMICS, MIN_CHUNKS> {
  ...
  fn call_task(&self, task: *const TaskObject<(), ATOMICS, MIN_CHUNKS>, signal: EmptySignal<ATOMICS, MIN_CHUNKS>) {
    ...
    (task_ref.work.unwrap())(self, task, LoopArguments{ work_size: &task_ref.work_size, work_indexes: &task_ref.work_indexes, work_indexes_index: &task_ref.work_indexes_index, empty_signal: signal, finish_count: &task_ref.finish_count});
    self.end_task(task); 
  }
