...
impl<const ATOMICS: usize, const MIN_CHUNKS: usize> TaskTrait for Task<ATOMICS, MIN_CHUNKS> {
  ...
  fn work_loop<'a, F: FnMut(u32)>(
    ...
    let atomics_length = loop_arguments.work_size.len() as u32;
    
    // Loop through atomics until finished
    while loop_arguments.finish_count.load(Ordering::Relaxed) < atomics_length {
      // Next atomic to work on
      let current_index = (loop_arguments.work_indexes_index.fetch_add(1, Ordering::Relaxed) % atomics_length) as usize;
      ...
      if chunk_idx == loop_arguments.work_size[current_index] {
        loop_arguments.finish_count.fetch_add(1, Ordering::Relaxed);
      }
    }
...
pub struct TaskObject<T, const ATOMICS: usize, const MIN_CHUNKS: usize> {
  ...
  pub(super) finish_count: AtomicU32,
  pub data: T,
}
